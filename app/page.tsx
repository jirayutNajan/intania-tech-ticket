...
export default function Page() {
  ...
  useEffect(() => {
    if (claimedTickets.ticket1 && claimedTickets.ticket2) {
      ...
      // After shaking, start merge animation
      setTimeout(() => {
        setShakeAnimation(false);
        setMergeAnimation(true);

        // After merge, start explosion
        setTimeout(() => {
          setExplosionAnimation(true);
          // Show team pass immediately after explosion
      ...
      <div className="-mt-4 flex h-1/2 w-full grow-[0.5] items-center justify-center">
        {showTeamPass ? (
          <div className="flex flex-col items-center justify-center animate-fadeIn">
            <div className="relative flex items-center justify-center team-pass-container" />
            <TeamPass className="scale-50 duration-500 ease-in-out hov  er:scale-[0.52] active:scale-[1.02] sm:scale-90 sm:hover:scale-[0.92] md:scale-100 md:hover:scale-[1.02]" />
          ...
          <div className="flex flex-col gap-10 md:flex-row lg:gap-24">
            ...
            <div className={cn(
              "relative flex aspect-[4/3] w-[320px] items-center gap-6 rounded-3xl border-2 p-6 backdrop-blur-sm sm:p-10 lg:w-[440px] transition-all duration-500",
              claimedTickets.ticket2 && "ticket-claimed-right",
              !claimedTickets.ticket2 && "border-white/40",
              shakeAnimation && "animate-shake",
              mergeAnimation && "animate-mergeSpiralRight",
              explosionAnimation && "animate-mergeExplosion"
            )}>
              <div className="flex h-full w-full flex-col items-center justify-center gap-3.5 sm:gap-6">
                {claimedTickets.ticket2 ? (
                  <>
                    <Settings className="text-white w-32 h-32" />
                    <div className="absolute bottom-4 text-white text-sm">
                      Expiring in 1 days 23 hours
                    </div>
                  </>
